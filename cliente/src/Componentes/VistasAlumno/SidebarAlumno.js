...
const SidebarAlumno = ({ setVista }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get(`https://201.124.152.8:3001/api/alumnos/obtenerNombreCompleto?userPk=${userPk}`);
    ...
    const fetchMedallas = async () => {
      try {
        const response = await axios.get(`https://201.124.152.8:3001/api/alumnos/obtenerMedallas?pkUsuario=${userPk}`);
