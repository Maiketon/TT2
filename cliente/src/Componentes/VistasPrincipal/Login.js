...
//////////////////////////////////////////////////////////////////////////////
  ...
  const limpiarCookies = () => {
    ...
    const enviarValores = () => {
      setEstaCargando(true);
      axios.post('https://201.124.152.8:3001/api/login/login', datosUsuario)
        .then(async (response) => {
    ...
    function actualizarEstatusUsuario(pkUsuario) {
      ...
      axios.post('https://201.124.152.8:3001/api/login/actualizarVerificacion', { pkUsuario })
          .then(({ data }) => {
              if (data.error) {
                  alert(data.error);
              } else {
  ...
  useEffect(() => {
    const fetchMaterias = async () => {
      try {
        const response = await axios.get('https://201.124.152.8:3001/api/alumnos/materias');
  ...
  setDeshabilitado(deshabilitado.map((item, i) => i === indice ? nuevaSeleccion[indice].seleccionado : item));
    ...
    const crearCheckboxes = (seleccion, columna) => {
      ...
      const guardarPreferencias = async (e) =>
      {
        ...
        const response = await axios.post('https://201.124.152.8:3001/api/alumnos/enviarPreferencias', cargaUtil);
...
const recuperarContra = async (e) => {
  ...
  try {
    setEstaCargando(true);
    const response = await axios.post('https://201.124.152.8:3001/api/alumnos/recuperacion', { correo });
